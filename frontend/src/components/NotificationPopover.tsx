...
const NotificationPopover = () => {
  ...
  const { data: unreadData } = useQuery({
    ...
    queryFn: async () => { const r = await notificationApi.getUnreadCount(); return r.data.data },
    refetchInterval: 30000,
  })

  const { data: listData, isLoading } = useQuery({
    queryKey: ['notifications-list'],
    queryFn: async () => { const r = await notificationApi.getList(0, 20); return r.data.data?.content || r.data.data || [] },
    enabled: open,
  })

  const readMut = useMutation({
    mutationFn: (id: number) => notificationApi.markAsRead(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['notifications-list', 'notifications-unread'] })
  })
  ...
  const unreadCount = typeof unreadData === 'number' ? unreadData : (unreadData as any)?.unreadCount || 0
  ...
  const content = (
    <div style={{ width: 360, maxHeight: 400, overflow: 'auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, padding: '0 4px' }}>
        ...
        {unreadCount > 0 && <Button type="link" size="small" onClick={() => readAllMut.mutate()}>全部已读</Button>}
      </div>
