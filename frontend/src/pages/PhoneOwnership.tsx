...
import { Table, Button, Card, Select, Space, Input, Modal, Tag, message, Upload, Row, Col, Statistic } from 'antd'
import { UploadOutlined, DownloadOutlined, EditOutlined } from '@ant-design/icons'
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'
import { phoneOwnershipApi, type PhoneOwnership, type ImportCompareItem } from '@/api/phoneOwnership'
...
const PhoneOwnershipPage = () => {
  ...
  const { data: listData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await phoneOwnershipApi.search({ keyword: keyword || undefined, branchOrgId, page, size })
      return res.data
    },
  })
  ...
  const columns = [
    ...
    { title: '分行', dataIndex: 'branchName', key: 'branchName', width: 150, render: (v: string) => v || '-' },
    { title: '部门', dataIndex: 'deptName', key: 'deptName', width: 150, render: (v: string) => v || '-' },
  ...
  const compareColumns = [
    ...
    {
      title: '分行(导入→现有)', key: 'branch', width: 200,
      render: (_: unknown, r: ImportCompareItem) => (
        <span>
          ...
          {r.hasDiff && r.existingBranchName !== r.branchName && (
            <><span style={{ color: '#999', margin: '0 4px' }}>←</span><span style={{ color: '#999', textDecoration: 'line-through' }}>{r.existingBranchName || '-'}</span></>
          )}
        </span>
      ),
    },
    {
      title: '部门(导入→现有)', key: 'dept', width: 200,
      render: (_: unknown, r: ImportCompareItem) => (
        <span>
          <span style={{ fontWeight: r.hasDiff ? 600 : 400 }}>{r.deptName || '-'}</span>
          {r.hasDiff && r.existingDeptName !== r.deptName && (
            <><span style={{ color: '#999', margin: '0 4px' }}>←</span><span style={{ color: '#999', textDecoration: 'line-through' }}>{r.existingDeptName || '-'}</span></>
          )}
        </span>
      ),
    },
    {
      title: '备注(导入→现有)', key: 'remark', width: 200,
      render: (_: unknown, r: ImportCompareItem) => (
        <span>
          <span style={{ fontWeight: r.hasDiff ? 600 : 400 }}>{r.remark || '-'}</span>
          {r.hasDiff && r.existingRemark !== r.remark && (
            <><span style={{ color: '#999', margin: '0 4px' }}>←</span><span style={{ color: '#999', textDecoration: 'line-through' }}>{r.existingRemark || '-'}</span></>
          )}
        </span>
      ),
    },
  ...
  return (
    <div>
      <Row gutter={16} style={{ marginBottom: 16 }}>
        ...
        <Col span={8}>
          <Card><Statistic title="已归属分行" value={content.filter(c => c.branchName).length} valueStyle={{ color: '#52c41a' }} /></Card>
        </Col>
        <Col span={8}>
          <Card><Statistic title="已归属部门" value={content.filter(c => c.deptName).length} valueStyle={{ color: '#1890ff' }} /></Card>
        </Col>
      </Row>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <Space>
            ...
            <Select placeholder="筛选分行" value={branchOrgId} onChange={v => { setBranchOrgId(v); setPage(0) }} style={{ width: 150 }} allowClear>
              {branches.map((b: any) => <Select.Option key={b.id} value={b.id}>{b.branchName || b.name}</Select.Option>)}
            </Select>
          </Space>
          <Space>
      ...
      >
        <div style={{ marginBottom: 16 }}>
          ...
          <Select
            ...
            placeholder="选择分行"
            value={editBranch}
            onChange={(v) => { setEditBranch(v); setEditDept(undefined); }}
            allowClear
          >
            {branches.map((b: any) => <Select.Option key={b.id} value={b.id}>{b.branchName || b.name}</Select.Option>)}
          </Select>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <Select
            ...
            allowClear
          >
            {depts.map((d: any) => <Select.Option key={d.id} value={d.id}>{d.name}</Select.Option>)}
          </Select>
        </div>
        <div>
      ...
      <Modal
        ...
        footer={[
          ...
          <Button key="confirm" type="primary" onClick={() => confirmImportMutation.mutate(compareData)} loading={confirmImportMutation.isPending}>
            确认导入
          </Button>,
        ]}
      >
