...
import { Table, Card, Input, Button, Space, Tag, Modal, Select, message, Popconfirm, Form } from 'antd'
import { SearchOutlined, ReloadOutlined, KeyOutlined, StopOutlined, CheckCircleOutlined, DeleteOutlined } from '@ant-design/icons'
      ...
      render: (_: any, record: UserVO) => (
        <Space size="small">
          ...
          {record.status === 'active' ? (
            ...
            <Button size="small" type="primary" icon={<CheckCircleOutlined />} onClick={() => handleToggleStatus(record.employeeId, 'enable')}>启用</Button>
          )}
          <Popconfirm title="确定删除此用户？" description="删除后不可恢复" onConfirm={() => handleDelete(record.employeeId)} okText="确定" cancelText="取消">
