...
import { Table, Card, Select, Tag, Space, Input, Dropdown, message, Row, Col, Statistic } from 'antd'
...
const PhoneManagement = () => {
  ...
  const columns = [
    ...
    {
      title: '使用人', dataIndex: 'employeeName', key: 'employeeName', width: 100,
      render: (v: string) => v || <span style={{ color: '#bfbfbf' }}>-</span>,
    },
    {
      title: '员工ID', dataIndex: 'employeeNo', key: 'employeeNo', width: 100,
      render: (v: string) => v || '-',
    },
    {
      title: '分机号', dataIndex: 'extensions', key: 'extensions', width: 160,
      render: (exts: string[]) =>
        exts.length > 0
          ? exts.map((e, i) => <Tag key={i} style={{ marginBottom: 2 }}>{e}</Tag>)
          : <span style={{ color: '#bfbfbf' }}>-</span>,
    },
    {
      title: 'MAC', dataIndex: 'macAddresses', key: 'macAddresses', width: 180,
      render: (macs: string[]) =>
        macs.length > 0
          ? macs.map((m, i) => <Tag key={i} color="cyan" style={{ marginBottom: 2, fontSize: 11 }}>{m}</Tag>)
          : <span style={{ color: '#bfbfbf' }}>-</span>,
    },
    {
      title: '分行', dataIndex: 'branchName', key: 'branchName', width: 100,
      render: (v: string) => v || '-',
    },
    {
    ...
    },
    {
      ...
      render: (s: number, record: PhoneView) => {
        ...
        return (
          <Dropdown
            menu={{ items: statusMenuItems, onClick: (info) => handleStatusMenuClick(record, info) }}
            trigger={['click']}
          >
            <Tag color={m.color} style={{ cursor: 'pointer' }}>{m.label}</Tag>
          </Dropdown>
        )
      },
    },
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <Space>
            <Input.Search
              ...
              style={{ width: 220 }}
            />
            <Select placeholder="状态筛选" value={filterStatus} onChange={v => { setFilterStatus(v); setPage(0) }} style={{ width: 130 }} allowClear>
              <Select.Option value={0}>闲置</Select.Option>
              <Select.Option value={1}>占用</Select.Option>
              <Select.Option value={2}>分配中</Select.Option>
            </Select>
            <Select placeholder="组织筛选" value={filterOrgId} onChange={v => { setFilterOrgId(v); setPage(0) }} style={{ width: 150 }} allowClear>
              {orgs.filter((o: any) => o.level >= 2).map((o: any) => (
