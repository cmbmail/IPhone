...
import { reconciliationApi, SubsidiaryReconciliation, ReconciliationSummary } from '@/api/reconciliation'
...
const SubsidiaryReconciliationPage = () => {
  ...
  const columns = [
    ...
    { title: '账单金额', dataIndex: 'totalBillAmount', key: 'totalBillAmount', width: 140,
      render: (val: number) => `¥${val?.toFixed(2) || '0.00'}` },
    { title: '发票金额', dataIndex: 'invoiceAmount', key: 'invoiceAmount', width: 140,
      render: (val: number) => `¥${val?.toFixed(2) || '0.00'}` },
    ...
    { title: '差异率', dataIndex: 'diffPercentage', key: 'diffPercentage', width: 100,
      render: (val: number) => {
        const pct = val || 0
  ...
  const totalAmount = reconciliations.reduce((sum: number, r: SubsidiaryReconciliation) => sum + (r.totalBillAmount || 0), 0)
  ...
  return (
    <div>
      <Row gutter={16} style={{ marginBottom: 16 }}>
        ...
        <Col span={6}>
          <Card><Statistic title="组织总数" value={summary?.totalOrgs || 0} /></Card>
        </Col>
        <Col span={6}>
