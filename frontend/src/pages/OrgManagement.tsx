...
import { Tree, Button, Modal, Form, Input, Select, Space, message, Popconfirm, Tooltip, Table, Tag, Upload } from 'antd'
import { EditOutlined, ApartmentOutlined, TeamOutlined, KeyOutlined, StopOutlined, DeleteOutlined, SwapOutlined, CheckCircleOutlined } from '@ant-design/icons'
...
const OrgManagement: React.FC = () => {
  ...
  React.useEffect(() => {
    roleApi.getActive().then(res => setActiveRoles(res.data?.data || [])).catch(() => {})
  }, [])
  ...
  React.useEffect(() => { fetchTree(); fetchAllUsers() }, [])

  const getAllKeys = (nodes: OrgStructure[]): string[] => {
    const keys: string[] = []
    const collect = (ns: OrgStructure[]) => { for (const n of ns) { keys.push(String(n.id)); if (n.children) collect(n.children) } }
    collect(nodes)
    return keys
  }
      ...
      return {
        ...
        title: (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '2px 0' }}>
            <ApartmentOutlined />
            <span style={{ fontWeight: isSelected ? 600 : 500, color: isSelected ? '#1677ff' : undefined }}>{node.name}</span>
            <Tooltip title="编辑">
        ...
        children: node.children && node.children.length > 0 ? convertToAntTree(node.children) : undefined,
      }
    })
  }

  const expandedAllKeys = useMemo(() => getAllKeys(treeData), [treeData])
  React.useEffect(() => { setExpandedKeys(expandedAllKeys) }, [expandedAllKeys])
  ...
  const openEditModal = (org: OrgStructure) => {
    setEditingOrg(org)
    let position = 1
    const siblings = org.parentId == null || org.parentId === undefined
      ? treeData
  ...
  const handleAdd = async () => {
    try {
      ...
      await orgApi.create({ parent_id: editingOrg?.id ?? null, name: values.name } as Record<string, unknown>)
      message.success(`已在「${editingOrg?.name || ''}」下添加「${values.name}」`)
      setAddChildOpen(false); setEditModalOpen(false)
      fetchTree()
    } catch { /* validation */ }
  }
  ...
  const handleEdit = async () => {
    ...
    try {
      const values = await orgForm.validateFields()
      await orgApi.update(editingOrg.id, { name: values.name, sort_order: Number(values.sortOrder) } as Record<string, unknown>)
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      if (selectedOrgId === id) { setSelectedOrgId(null); setSelectedOrgName(''); setUserList([]) }
      fetchTree()
    } catch (e: any) {
    ...
    if (selectedKeys.length > 0) {
      ...
      const findName = (nodes: OrgStructure[]): string => {
        for (const n of nodes) {
          ...
          if (n.children) { const found = findName(n.children); if (found) return found }
        }
        return ''
      }
  ...
  const userColumns = [
    ...
    {
      title: '账号', dataIndex: 'username', key: 'username', width: 130, align: 'center',
    ...
    {
      title: '角色', dataIndex: 'roleName', key: 'roleName', width: 100, align: 'center',
      render: (v: string) => v || '-'
    },
    {
      title: '修改时间', dataIndex: 'updatedAt', key: 'updatedAt', width: 170, align: 'center',
    ...
    },
    {
      ...
      render: (_: any, record: UserVO) => (
        <Button type="link" size="small" onClick={() => openUserEdit(record)}>操作</Button>
      )
    }
  ]
  ...
  return (
    <div style={{ padding: 0 }}>
      ...
      <div style={{ display: 'flex', gap: 16 }}>
        ...
        <div style={{
          flex: '0 0 280px',
          background: '#fff', borderRadius: 8, padding: '16px 8px',
      ...
      {/* Edit Org Modal */}
      <Modal
        title={addChildOpen ? `在「${editingOrg?.name}」下添加子机构` : `编辑「${editingOrg?.name}」`}
        open={editModalOpen}
        onCancel={() => { setEditModalOpen(false); setAddChildOpen(false); setEditModalOpen(false) }}
        footer={null}
        destroyOnClose
        width={440}
      >
        <Form form={orgForm} layout="vertical" preserve={false}>
          <Form.Item label="机构名称" required>
            <div style={{ display: 'flex', gap: 8 }}>
              <Form.Item name="name" noStyle rules={[{ required: true, message: '请输入机构名称' }]}>
                <Input ref={nameInputRef} placeholder="请输入机构名称" autoComplete="off" />
              </Form.Item>
              {!addChildOpen && (
                <Button type="dashed" onClick={openAddChildForm}>添加子机构</Button>
              )}
            </div>
          </Form.Item>
          {!addChildOpen && (
            <>
            <Form.Item name="sort_order" label="排序位置" rules={[{ required: true, message: '请输入排序位置' }]}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
          ...
          <Space>
            {!addChildOpen && (
              <Popconfirm title={`确定删除「${editingOrg?.name}」？`} description="请先确认该机构下无人员关联" onConfirm={() => { if (editingOrg) { handleDelete(editingOrg.id, editingOrg.name); setEditModalOpen(false) } }}>
      ...
      <Modal
        ...
        open={editUserOpen}
        onCancel={() => { setEditUserOpen(false); setEditingUser(null) }}
        footer={null}
        destroyOnClose
        width={440}
      >
        ...
        <div style={{ marginTop: 24, padding: '12px 0', borderTop: '1px solid #f0f0f0' }}>
          <Popconfirm
            ...
            onConfirm={() => {
              if (editingUser) { handleDeleteUser(editingUser); setEditUserOpen(false) }
            }}
            okText="删除"
            cancelText="取消"
            okButtonProps={{ danger: true }}
          >
            <Button type="text" danger icon={<DeleteOutlined />}>删除此账号</Button>
          </Popconfirm>
        </div>
      </Modal>
    </div>
