...
import { Table, Button, Card, Select, Tag, Space, Modal, Form, Input, message, Drawer, Typography } from 'antd'
...
const AnnouncementManagement = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: Record<string, unknown> }) => announcementApi.update(id, data),
    onSuccess: () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'type',
      width: 120,
      render: (type: string) => <Tag color={TYPE_COLORS[type] || 'default'}>{TYPE_NAMES[type] || type}</Tag>
    },
    {
      title: '优先级',
      dataIndex: 'priority',
      key: 'priority',
      width: 100,
      render: (p: string) => <Tag color={PRIORITY_COLORS[p] || 'default'}>{PRIORITY_NAMES[p] || p}</Tag>
    },
    {
      title: '状态',
      dataIndex: 'status',
      ...
      render: (_: unknown, record: Announcement) => (
        <Space size="small">
          ...
          <Button size="small" danger onClick={() => {
            Modal.confirm({ title: '确认删除', content: `删除公告「${record.title}」？`, onOk: () => deleteMutation.mutate(record.id) })
          }}>删除</Button>
        </Space>
      )
    }
        ...
        {selectedRecord && (
          <div>
            <Space style={{ marginBottom: 16 }}>
              <Tag color={TYPE_COLORS[selectedRecord.announcementType]}>{TYPE_NAMES[selectedRecord.announcementType]}</Tag>
              <Tag color={PRIORITY_COLORS[selectedRecord.priority]}>{PRIORITY_NAMES[selectedRecord.priority]}</Tag>
              <Tag color={STATUS_COLORS[selectedRecord.status]}>{STATUS_NAMES[selectedRecord.status]}</Tag>
            </Space>
